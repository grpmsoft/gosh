...
func TestFileHistoryRepository_Concurrency(t *testing.T) {
	t.Run("handles concurrent saves", func(t *testing.T) {
		...
		// IMPORTANT: Give time for all file descriptors to close on Windows.
		// Windows file locking can prevent TempDir cleanup if files are still open.
		// This prevents flaky test failures on Windows CI (GitHub Actions).
